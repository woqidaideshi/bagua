...
def main_worker(args):
    ...
    log_name = "imagenet_" + args.algorithm + "_lr" + str(args.lr) + "_gpu" + str(bagua.get_world_size()) + "_rank" + str(bagua.get_rank()) + "_" + get_current_time_filename() + ".txt"
    outdir = os.path.join("./log", "sparsepy", "imagenet", args.algorithm)
    ...
    val_tuple_num = len(val_loader.dataset)
    ...
    for epoch in range(args.start_epoch, epochs):
        ...
        # train for one epoch
        (start_time, grad_end_t) = train(train_loader, model, criterion, optimizer, scaler, epoch, args)

        if args.algorithm == "async":
            model.bagua_algorithm.abort(model)

        # evaluate on validation set
        (epoch_loss, acc1, loss_end_t)  = validate(val_loader, model, criterion, epoch, args)
    ...
    logging.info("----test main end time: {} (rank {}).".format(end_date.strftime("%Y-%m-%d %H:%M:%S.%f"), bagua.get_rank()))
...
def validate(val_loader, model, criterion, epoch, args):
    ...
    end_log = time.time()
    return (losses.avg, top1.avg, end_log)
