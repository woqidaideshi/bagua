...
class BaguaBucket:
    def __init__(
        ...
        logger.info("-----------------------bucket tensors element num: {}, bagua tensor num: {}".format(sum(tensor.numel() for tensor in self.tensors), sum(tensor.bagua_getter_closure().numel() for tensor in self.tensors)))
        if alignment > 1:
    ...
    def append_centralized_sparse_inplace_parallel_synchronous_op(
        self,
        ...
        if hierarchical:
            self.backend_bucket.append_centralized_sparse_inplace_parallel_synchronous_op(
                _bagua_backend_comm(group.get_inter_node_communicator()),
                _bagua_backend_comm(group.get_intra_node_communicator()),
                hierarchical=hierarchical,
                compression=compression,
    ...
    def append_centralized_sparse_py_cuda_parallel_synchronous_op(
        self,
        ...
        if hierarchical:
            self.backend_bucket.append_centralized_sparse_py_cuda_parallel_synchronous_op(
                _bagua_backend_comm(group.get_inter_node_communicator()),
                _bagua_backend_comm(group.get_intra_node_communicator()),
                hierarchical=hierarchical,
                compression=compression,
    ...
    def append_centralized_sparse_py_cuda_synchronous_op(
        ...
        if hierarchical:
            self.backend_bucket.append_centralized_sparse_py_cuda_synchronous_op(
                _bagua_backend_comm(group.get_inter_node_communicator()),
                _bagua_backend_comm(group.get_intra_node_communicator()),
                hierarchical=hierarchical,
                compression=compression,
    ...
    def append_centralized_sparse_py_synchronous_op(
        self,
        recv_value: BaguaTensor,
        recv_index: BaguaTensor,
        send_value: BaguaTensor,
        ...
        if hierarchical:
            self.backend_bucket.append_centralized_sparse_py_synchronous_op(
                _bagua_backend_comm(group.get_inter_node_communicator()),
                _bagua_backend_comm(group.get_intra_node_communicator()),
                hierarchical=hierarchical,
                compression=compression,
    ...
    def append_centralized_sparse_synchronous_op(
        ...
        if hierarchical:
            self.backend_bucket.append_centralized_sparse_synchronous_op(
                ...
                hierarchical=hierarchical,
                compression=compression,
                value_tensor=value_tensor.bagua_backend_tensor(),
                other_tensor=other_tensor.bagua_backend_tensor(),
            )
        else:
            self.backend_bucket.append_centralized_sparse_synchronous_op(
